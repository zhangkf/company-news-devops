...
class admin inherits base {
  package { ["java-1.6.0-openjdk-devel", "git", "ant", "ant-junit", "ruby-devel", "openssl-devel", "make", "gcc", "libxml2-devel", "libxslt-devel"]:
    ensure => "present"
  }
  ...
  file { "/etc/go/cruise-config.xml":
    ...
    group   => "go",
    mode    => 664,
    content => template("cruise-config.xml.erb"),
